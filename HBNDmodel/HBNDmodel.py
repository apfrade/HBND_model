...
def get_descriptors(mols):
    ...
    descriptor_names = pd.read_pickle('HBNDmodel/data/descriptor_names')
...
def scale_df(df):
    ...
    # load scaling model
    scaler_model = load('HBNDmodel/data/scaler_model.bin')
...
def predict(scaled_df, report=False):
    ...
    # load hbnd model
    hbnd_model =  pickle.load(open('HBNDmodel/data/hbnd_model.sav', "rb"))
